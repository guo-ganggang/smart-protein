...
# Text corpus for sentence cleaning
def clean_literatures_corpus(dir_path):
    # Load the punkt tokenizer
    download_nltk_corpus_path = dir_path + 'nltk_data/'
    if not os.path.exists(download_nltk_corpus_path):
        os.makedirs(download_nltk_corpus_path)
        nltk.download("stopwords") # 下载到上面那个路径下
    data.path.append(download_nltk_corpus_path)
    english_stopwords = stopwords.words('english') # 自动检索上面那个路径
    english_punctuations = [',', '.', ':', ';', '?', '(', ')', '[', ']', '&', '!', '*', '@', '#', '$', '%',
                            '>', '<', '\'s', '\'', '-/-', '``', "''", '--', '-']

    raw_text_file_path = dir_path + 'corpus_literatures_title_abstract.csv'
    clean_text_file_path = dir_path + 'corpus_literatures_title_abstract_clean.csv'
    with open(clean_text_file_path, 'w') as output_file:
        with codecs.open(raw_text_file_path, "rb", "utf-8") as input_file:
            for line in input_file:
                corpus = line.strip().split('$')[1].strip()
...
def clean_mesh_words(dir_path):
    ...
    raw_text_file_path = dir_path + 'protein_official_symbol_pmids_mesh_words_for_train_modify_mesh_terms.csv'
    clean_text_file_path = dir_path + 'protein_official_symbol_pmids_mesh_words_for_train_modify_mesh_terms_clean.csv'
    with open(clean_text_file_path, 'w') as output_file:
        with codecs.open(raw_text_file_path, "rb", "utf-8") as input_file:
            for line in input_file:
...
def train_w2v_model(dir_path):
    ...
    clean_text_corpus = dir_path + 'corpus_literatures_title_abstract_clean.csv'
    if (not os.path.isfile(clean_text_corpus)) or (os.path.getsize(clean_text_corpus) < 100000):
    ...
    clean_mesh_topic_words = dir_path + 'protein_official_symbol_pmids_mesh_words_for_train_modify_mesh_terms_clean.csv'

    if (not os.path.isfile(clean_mesh_topic_words)) or (os.path.getsize(clean_mesh_topic_words) < 100):
        print ('INFORM : There is no mesh_words file. Generate mesh_words file from fasta file...')
        clean_mesh_words(dir_path)
    else:
        print("INFORM : File's Existence is confirmed")
    ...
    out_model = dir_path + 'w2v_model/human_protein_literature_corpus_sg1_s128_w5_m1.bin'
    out_vec = dir_path + 'w2v_model/human_protein_literature_corpus_sg1_s128_w5_m1.vector'
