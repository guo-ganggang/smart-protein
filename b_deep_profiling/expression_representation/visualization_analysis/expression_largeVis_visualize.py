...
def protein_space(dir_path):
    ...
    vector_class = dh.read_protein_atlas_info(dir_path)
    ...
    file_path_pkl = dir_path + 'visualize/gene_expression_3D_vector.pkl'
    ...
    cm = plt.cm.get_cmap(color_sets[7]) #7
    print("Visualization:Normalized distributions of biochemical and biophysical properties in protein-space")
    fig_path = dir_path + 'visualize/Distribution of chromosome proterties in gene-expression-space.pdf' # 'custom'
    ...
    sc = ax.scatter(vectors[:, 0], vectors[:, 1], vectors[:, 2], alpha=0.4, s=marker_size, c=property_list, cmap=cm) # cm,set_color
    # ax.set_title("Distribution of chromosome in expression-space")
